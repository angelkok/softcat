...
describe TeamsController do
  ...
  describe "DELETE destroy" do
    ...
    let!(:team) { create(:team) }
    let(:delete_attributes) {{id: team.id}}

    it"redirects to :index" do
      make_request(delete_attributes)
      expect(response.code).to redirect_to(teams_path)
    end

    it "calls Team#remove_players_from_team" do
      Team.any_instance.should_receive(:remove_players_from_team)
      make_request(delete_attributes)
    end

    it "destroys team object" do
      expect { make_request(delete_attributes) }.to change(Team, :count).by(-1)
    end
  end
end
